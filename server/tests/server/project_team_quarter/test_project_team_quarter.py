...
def test_when_project_team_quarter_has_change_request_and_incident_both_are_summed_up():
    ...
    executed = graphene_client.execute(
        """
            query ProjectTeamQuarterByQuarterKeyAndProjectTeamId($quarterKey: String, $projectTeamId: Int) {
                projectTeamQuarterByQuarterKeyAndProjectTeamId(quarterKey: $quarterKey, projectTeamId: $projectTeamId) {
                    quarter {
                        key
                    }
        ...
        """,
        variables={"quarterKey": quarter_key, "projectTeamId": project_team_id},
    )
    assert executed == {
        "data": {
