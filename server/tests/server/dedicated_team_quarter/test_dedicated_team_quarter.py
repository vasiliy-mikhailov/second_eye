...
def test_when_dedicated_team_quarter_has_change_request_and_incident_both_are_summed_up():
    ...
    executed = graphene_client.execute(
        """
            query DedicatedTeamQuarterByQuarterKeyAndDedicatedTeamId($quarterKey: String, $dedicatedTeamId: Int) {
                dedicatedTeamQuarterByQuarterKeyAndDedicatedTeamId(quarterKey: $quarterKey, dedicatedTeamId: $dedicatedTeamId) {
                    quarter {
                        key
                    }
        ...
        """,
        variables={"quarterKey": quarter_key, "dedicatedTeamId": dedicated_team_id},
    )
    assert executed == {
        "data": {
