...
def test_planning_period_persons_list():
    ...
    executed = graphene_client.execute(
        ...
        """,
        variables={"id": two_weeks_ago_year},
    )
    assert executed == {
        "data": {
            "planningPeriodById": {
                "persons": [{
                    "personId": person1_id,
                    "personKey": person1_key,
                    "personName": "Исполнитель 1",
                    "functionPointsEffort": 5 + 4,
                    "effortPerFunctionPoint": pytest.approx((5 * (5 + 10) / 10 + 4 * (4 + 6) / 15) / (5 + 4)),
                }, {
                    "personId": person2_id,
                    "personKey": person2_key,
                    "personName": "Исполнитель 2",
                    "functionPointsEffort": 10 + 6,
                    "effortPerFunctionPoint": pytest.approx((10 * (5 + 10) / 10 + 6 * (4 + 6) / 15) / (10 + 6)),
                }],
            }
        }
    }
