def calculate_dedicated_team_planning_period_time_spent_percent_with_value_and_without_value_by_date(task_time_sheets):
    ...
    result = result[["dedicated_team_planning_period_id", "date", "time_spent", "time_spent_with_value", "time_spent_without_value"]].groupby(
        ["dedicated_team_planning_period_id", "date"]
    ).agg({"time_spent_with_value": "sum", "time_spent_without_value": "sum"}).reset_index()
    result = result.sort_values(["date"]).reset_index(drop=True)

    result["time_spent_with_value_cumsum"] = result.groupby(["dedicated_team_planning_period_id"])["time_spent_with_value"].cumsum(axis=0)
    result["time_spent_without_value_cumsum"] = result.groupby(["dedicated_team_planning_period_id"])["time_spent_without_value"].cumsum(axis=0)
    ...
    result = result[["dedicated_team_planning_period_id", "date", "time_spent_with_value_percent_cumsum", "time_spent_without_value_percent_cumsum"]]
